...
class CAR_Airboat : public CPropVehicleDriveable
{
	DECLARE_CLASS(CAR_Airboat, CPropVehicleDriveable);
...
IMPLEMENT_SERVERCLASS_ST(CAR_Airboat, DT_PropAirboatAR)
...
LINK_ENTITY_TO_CLASS(prop_vehicle_ar_airboat, CAR_Airboat);

BEGIN_DATADESC(CAR_Airboat)
...
// Purpose: 
//-----------------------------------------------------------------------------
void CAR_Airboat::Precache(void)
{
	BaseClass::Precache();
...
// Purpose: 
//-----------------------------------------------------------------------------
void CAR_Airboat::Spawn(void)
{
	DevMsg("CPropAirboat::Spawn\n");
...
//-----------------------------------------------------------------------------
void CAR_Airboat::CreateAntiFlipConstraint()
{
...
// Attachment indices
//-----------------------------------------------------------------------------
void CAR_Airboat::UpdateOnRemove()
{
...
void CAR_Airboat::CreatePlayerBlocker()
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CAR_Airboat::DestroyPlayerBlocker()
{
	if (m_hPlayerBlocker != NULL)
	{
...
// Input  : bEnable - 
//-----------------------------------------------------------------------------
void CAR_Airboat::EnablePlayerBlocker(bool bEnable)
{
	if (m_hPlayerBlocker != NULL)
	{
		if (bEnable)
...
void CAR_Airboat::UpdateWeaponSound()
{
	if (HasGun())
	{
...
//-----------------------------------------------------------------------------
void CAR_Airboat::InputExitVehicle(inputdata_t &inputdata)
{
	m_bForcedExit = true;
}
...
//			in d1_canals_10b.
//-----------------------------------------------------------------------------
void CAR_Airboat::InputWake(inputdata_t &inputdata)
{
...
//-----------------------------------------------------------------------------
void CAR_Airboat::InputEnableGun(inputdata_t &inputdata)
{
...
// Purpose: Input handler to enable or disable the airboat's mounted gun.
//-----------------------------------------------------------------------------
void CAR_Airboat::InputStartRotorWashForces(inputdata_t &inputdata)
{
...
// Purpose: Input handler to enable or disable the airboat's mounted gun.
//-----------------------------------------------------------------------------
void CAR_Airboat::InputStopRotorWashForces(inputdata_t &inputdata)
{
...
// Used for navigation
//-----------------------------------------------------------------------------
void CAR_Airboat::EnterVehicle(CBaseCombatCharacter *pPlayer)
{
...
//-----------------------------------------------------------------------------
void CAR_Airboat::PreExitVehicle(CBaseCombatCharacter *pPlayer, int nRole)
{
	if (HeadlightIsOn())
	{
		HeadlightTurnOff();
...
// Input  : iRole - 
//-----------------------------------------------------------------------------
void CAR_Airboat::ExitVehicle(int nRole)
{
	CBaseEntity *pDriver = GetDriver();
...
// Purpose: 
//-----------------------------------------------------------------------------
void CAR_Airboat::HeadlightTurnOn(void)
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CAR_Airboat::HeadlightTurnOff(void)
{
...
// position to shoot at
//-----------------------------------------------------------------------------
Vector CAR_Airboat::BodyTarget(const Vector &posSrc, bool bNoisy)
{
	Vector vecPosition;
	QAngle angles;
...
Vector CAR_Airboat::GetSmoothedVelocity(void)
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CAR_Airboat::TraceAttack(const CTakeDamageInfo &inputInfo, const Vector &vecDir, trace_t *ptr, CDmgAccumulator *pAccumulator)
{
...
//-----------------------------------------------------------------------------
bool CAR_Airboat::ShouldDrawWaterImpacts(void)
{
...
// Allows the shooter to change the impact effect of his bullets
//-----------------------------------------------------------------------------
void CAR_Airboat::DoImpactEffect(trace_t &tr, int nDamageType)
{
	// The airboat spits out so much crap that we need to do cheaper versions
	// of the impact effects. Also, we need to do less of them.
	if (m_flLastImpactEffectTime == gpGlobals->curtime)
...
// Purpose: 
//-----------------------------------------------------------------------------
int CAR_Airboat::OnTakeDamage(const CTakeDamageInfo &info)
{
...
// Scraping noises for the various things we drive on. 
//-----------------------------------------------------------------------------
void CAR_Airboat::VPhysicsFriction(IPhysicsObject *pObject, float energy, int surfaceProps, int surfacePropsHit)
{
...
#pragma optimize("", off)
void CAR_Airboat::AimGunAt(const Vector &aimPos, float flInterval)
{
...
// Removes the ammo...
//-----------------------------------------------------------------------------
void CAR_Airboat::RemoveAmmo(float flAmmoAmount)
{
...
// Recharges the ammo...
//-----------------------------------------------------------------------------
void CAR_Airboat::RechargeAmmo(void)
{
	if (!m_bHasGun)
	{
		m_nAmmoCount = -1;
...
// Purpose: 
//-----------------------------------------------------------------------------
void CAR_Airboat::ComputeAimPoint(Vector *pVecAimPoint)
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CAR_Airboat::UpdatePropeller()
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CAR_Airboat::StopLoopingSounds()
{
	CSoundEnvelopeController &controller = CSoundEnvelopeController::GetController();
...
//-----------------------------------------------------------------------------
void CAR_Airboat::UpdateEngineSound(CSoundEnvelopeController &controller, float speedRatio)
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CAR_Airboat::UpdateFanSound(CSoundEnvelopeController &controller, float speedRatio)
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CAR_Airboat::UpdateWaterSound(CSoundEnvelopeController &controller, float speedRatio)
{
...
// Purpose:
//-----------------------------------------------------------------------------
void CAR_Airboat::UpdateSplashEffects(void)
{
	// Splash effects.
...
// Purpose: 
//-----------------------------------------------------------------------------
const char *CAR_Airboat::GetTracerType(void)
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CAR_Airboat::DoMuzzleFlash(void)
{
	CEffectData data;
...
// Purpose: 
//-----------------------------------------------------------------------------
void CAR_Airboat::DriveVehicle(float flFrameTime, CUserCmd *ucmd, int iButtonsDown, int iButtonsReleased)
{
	if (ucmd->impulse == 100)
	{
		if (HeadlightIsOn())
...
//			*pMoveData - 
//-----------------------------------------------------------------------------
void CAR_Airboat::ProcessMovement(CBasePlayer *pPlayer, CMoveData *pMoveData)
{
...
//-----------------------------------------------------------------------------
void CAR_Airboat::CreateDangerSounds(void)
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CAR_Airboat::DampenEyePosition(Vector &vecVehicleEyePos, QAngle &vecVehicleEyeAngles)
{
...
//-----------------------------------------------------------------------------
void CAR_Airboat::ComputePDControllerCoefficients(float *pCoefficientsOut,
...
// Purpose:
//-----------------------------------------------------------------------------
void CAR_Airboat::DampenForwardMotion(Vector &vecVehicleEyePos, QAngle &vecVehicleEyeAngles, float flFrameTime)
{
	// Get forward vector.
	Vector vecForward;
...
// Purpose:
//-----------------------------------------------------------------------------
void CAR_Airboat::DampenUpMotion(Vector &vecVehicleEyePos, QAngle &vecVehicleEyeAngles, float flFrameTime)
{
	// Get up vector.
	Vector vecUp;
...
// Purpose: 
//-----------------------------------------------------------------------------
void CAR_Airboat::CreateSplash(int nSplashType)
{
	if (GetWaterLevel() == 0)
		return;
...
//-----------------------------------------------------------------------------
void CAR_Airboat::VPhysicsUpdate(IPhysicsObject *pPhysics)
{
...
//-----------------------------------------------------------------------------
float CAR_Airboat::CalculatePhysicsStressDamage(vphysics_objectstress_t *pStressOut, IPhysicsObject *pPhysics)
{
...
//-----------------------------------------------------------------------------
void CAR_Airboat::ApplyStressDamage(IPhysicsObject *pPhysics)
{
	vphysics_objectstress_t stressOut;
