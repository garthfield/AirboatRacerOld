...
ConVar r_AirboatViewBlendToAR("r_AirboatViewBlendTo", "1", FCVAR_CHEAT);
ConVar r_AirboatViewBlendToScaleAR("r_AirboatViewBlendToScale", "0.03", FCVAR_CHEAT);
ConVar r_AirboatViewBlendToTimeAR("r_AirboatViewBlendToTime", "1.5", FCVAR_CHEAT);

ConVar cl_draw_airboat_wakeAR("cl_draw_airboat_wake", "1", FCVAR_CHEAT);

// Curve parameters for pitch/roll blending.
// NOTE: Must restart (or create a new airboat) after changing these cvars!
ConVar r_AirboatRollCurveZeroAR("r_AirboatRollCurveZero", "90.0", FCVAR_CHEAT);			// Roll less than this is clamped to zero.
ConVar r_AirboatRollCurveLinearAR("r_AirboatRollCurveLinear", "120.0", FCVAR_CHEAT);	// Roll greater than this is mapped directly.
// Spline in between.

ConVar r_AirboatPitchCurveZeroAR("r_AirboatPitchCurveZero", "25.0", FCVAR_CHEAT);		// Pitch less than this is clamped to zero.
ConVar r_AirboatPitchCurveLinearAR("r_AirboatPitchCurveLinear", "60.0", FCVAR_CHEAT);	// Pitch greater than this is mapped directly.
// Spline in between.

ConVar airboat_joy_response_moveAR("airboat_joy_response_move", "1");					// Quadratic steering response
...
//
class C_AR_Airboat : public C_PropVehicleDriveable
{
	DECLARE_CLASS(C_AR_Airboat, C_PropVehicleDriveable);
...
IMPLEMENT_CLIENTCLASS_DT(C_AR_Airboat, DT_PropAirboatAR, CAR_Airboat)
...
// Purpose: Constructor
//-----------------------------------------------------------------------------
C_AR_Airboat::C_AR_Airboat()
{
	m_vecEyeSpeed.Init();
	m_flViewAngleDeltaTime = 0.0f;
	m_pHeadlight = NULL;

	m_ViewSmoothingData.flPitchCurveZero = r_AirboatPitchCurveZeroAR.GetFloat();
	m_ViewSmoothingData.flPitchCurveLinear = r_AirboatPitchCurveLinearAR.GetFloat();
	m_ViewSmoothingData.flRollCurveZero = r_AirboatRollCurveZeroAR.GetFloat();
	m_ViewSmoothingData.flRollCurveLinear = r_AirboatRollCurveLinearAR.GetFloat();
...
//-----------------------------------------------------------------------------
int C_AR_Airboat::GetPrimaryAmmoType() const
{
	if (m_nAmmoCount < 0)
		return -1;
...
int C_AR_Airboat::GetPrimaryAmmoCount() const
{
	return m_nAmmoCount;
}

bool C_AR_Airboat::PrimaryAmmoUsesClips() const
{
	return false;
}

int C_AR_Airboat::GetPrimaryAmmoClip() const
{
	return -1;
}
...
//-----------------------------------------------------------------------------
int C_AR_Airboat::GetJoystickResponseCurve() const
{
	return airboat_joy_response_moveAR.GetInt();
}

//-----------------------------------------------------------------------------
// Draws crosshair in the forward direction of the boat
//-----------------------------------------------------------------------------
void C_AR_Airboat::DrawHudElements()
{
...
//-----------------------------------------------------------------------------
void C_AR_Airboat::UpdateViewAngles(C_BasePlayer *pLocalPlayer, CUserCmd *pCmd)
{
	if (r_AirboatViewBlendToAR.GetInt())
	{
		//
		...
		if (m_flViewAngleDeltaTime > r_AirboatViewBlendToTimeAR.GetFloat())
		{
			...
			QAngle outAngles;
			InterpolateAngles(pCmd->viewangles, vehicleEyeAngles, outAngles, r_AirboatViewBlendToScaleAR.GetFloat());
...
// Purpose:
//-----------------------------------------------------------------------------
void C_AR_Airboat::DampenEyePosition(Vector &vecVehicleEyePos, QAngle &vecVehicleEyeAngles)
{
...
//-----------------------------------------------------------------------------
void C_AR_Airboat::ComputePDControllerCoefficients(float *pCoefficientsOut,
...
// Purpose:
//-----------------------------------------------------------------------------
void C_AR_Airboat::DampenForwardMotion(Vector &vecVehicleEyePos, QAngle &vecVehicleEyeAngles, float flFrameTime)
{
...
// Purpose:
//-----------------------------------------------------------------------------
void C_AR_Airboat::DampenUpMotion(Vector &vecVehicleEyePos, QAngle &vecVehicleEyeAngles, float flFrameTime)
{
	// Get up vector.
	Vector vecUp;
...
// Purpose: 
//-----------------------------------------------------------------------------
void C_AR_Airboat::OnEnteredVehicle(C_BasePlayer *pPlayer)
{
	int eyeAttachmentIndex = LookupAttachment("vehicle_driver_eyes");
	Vector vehicleEyeOrigin;
	QAngle vehicleEyeAngles;
...
//-----------------------------------------------------------------------------
void C_AR_Airboat::UpdateHeadlight()
{
	if (m_bHeadlightIsOn)
	{
		if (!m_pHeadlight)
...
// Purpose: 
//-----------------------------------------------------------------------------
void C_AR_Airboat::UpdateWake(void)
{
...
// Input  : &beamSeg - 
//-----------------------------------------------------------------------------
void C_AR_Airboat::DrawSegment(const BeamSeg_t &beamSeg, const Vector &vNormal)
{
	// Build the endpoints.
...
// Input  : position - 
//-----------------------------------------------------------------------------
void C_AR_Airboat::DrawPontoonSplash(Vector origin, Vector direction, float speed)
{
	Vector	offset;
...
//			wakeLength - 
//-----------------------------------------------------------------------------
void C_AR_Airboat::DrawPontoonWake(Vector	startPos, Vector wakeDir, float wakeLength, float speed)
{
#define	WAKE_STEPS	6
...
// Output : int
//-----------------------------------------------------------------------------
int C_AR_Airboat::DrawWake(void)
{
	if (cl_draw_airboat_wakeAR.GetBool() == false)
		return 0;
...
// Output : int
//-----------------------------------------------------------------------------
int C_AR_Airboat::DrawModel(int flags)
{
